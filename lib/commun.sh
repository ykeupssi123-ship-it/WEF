#!/bin/bash
# lib/commun.sh - fonctions partagees entre orchestrator.sh, bin/monitor.sh,
# bin/order.sh, bin/hold.sh, bin/free.sh. Ajoute le 2026-08-12.
#
...
# Control-M/Autosys. Voir bin/hold.sh / bin/free.sh.
...
# par orchestrator.sh/bin/order.sh vers tous les jobs qu'ils lancent).
local_pki_copy(){
  local target_dir="$1"
  local owner="$2"
  mkdir -p "$target_dir"
...
# depend reellement). Distinct de bin/confirm.sh : ici, on n'attend
...
# deja evite pour SMTP_PASS_FILE (voir bin/notify.sh) - ce point unique
