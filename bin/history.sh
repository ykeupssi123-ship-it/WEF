#!/bin/bash
# bin/history.sh - consulte l'historique COMPLET d'un job (toutes ses
...
#
# Usage :
#   ./bin/history.sh                          -> liste les jobs ayant un historique
#   ./bin/history.sh ES_017                    -> liste toutes les executions de ES_017
#   ./bin/history.sh ES_017 3                   -> affiche le log de la 3e execution (ordre chronologique)
#   ./bin/history.sh ES_017 20260812_104500     -> affiche le log dont le timestamp contient ce texte
#   ./bin/history.sh ES_017 stats               -> statistiques (nb executions, taux de reussite,
#                                                       intervalles entre executions - utile pour verifier
#                                                       qu'un job cense tourner regulierement le fait bien)
#   ./bin/history.sh ES_017 stats 300           -> pareil, + signale les intervalles qui s'ecartent de
...
if [ "$SEL" = "stats" ]; then
  ...
  # FORCE_OK/FORCE_ECHEC (bin/order.sh, ajoute le 2026-08-12) comptent
