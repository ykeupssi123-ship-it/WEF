...
# pwreset.sh est le SEUL point sanctionne pour la
...
# sur un cluster deja initialise, et es_admin_curl/pwreset.sh
...
if [ "$ADD_CODE" -ne 0 ] && echo "$ADD_OUT" | grep -q "changing the file owner"; then
  ...
  echo "[ES_022] Non bloquant : es_admin_curl/pwreset.sh rattraperont une eventuelle desynchronisation reelle au premier appel admin."
