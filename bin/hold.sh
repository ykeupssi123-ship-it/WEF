#!/bin/bash
# bin/hold.sh - Gel manuel (HELD) d'un job, ajoute le 2026-08-12.
...
if [ -z "$JOB_ID" ] || [ -z "$RAISON" ]; then
  echo "Usage : ./bin/hold.sh <JOB_ID> \"<raison>\""
...
echo "dependances sont satisfaites. Pour le liberer : ./bin/free.sh $JOB_ID"
