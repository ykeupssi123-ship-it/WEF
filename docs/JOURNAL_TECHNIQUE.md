...
## 2026-09-18 (suite) - Jobs RUN : `REPEATABLE_JOBS` elargi, jamais une derivation automatique

**Demande explicite** : "chez Control-M, une condition qu'un job pose a sa reussite ne sert jamais a bloquer CE MEME job... un job RUN n'a pas besoin, ils sont senses etre les employes payes pour le maintien en condition operationnelle, donc inconcevable que pour les jouer on doit supprimer un fichier de statut ok".

**Idee explorée puis REJETEE avant tout code** : deriver automatiquement un type `BUILD`/`RUN` depuis le segment deja present dans `JOB_NAME` (`WEF_<COMP>_BLD_...` / `WEF_<COMP>_RUN_...`, present sur 100% des 296 lignes, verifie par script). Seduisant (zero migration), mais **faux a l'usage** : `ES_027` a `ES_041` (15 jobs) sont tous nommes `..._RUN_...` alors qu'ils forment une chaine de construction lineaire stricte (`ES_035` produit `FACTORY_ES_READY`, consomme par `ES_036`, etc.) - clairement des etapes de construction UNIQUES. Le segment `RUN` du nom veut dire ici "execute une commande/verification" (par opposition a `BLD`="installe/ecrit une config"), pas "job operationnel rejouable a volonte" - une distinction differente de celle demandee. Deriver automatiquement aurait rendu ~90 jobs de construction rejouables sans blocage par erreur, un vrai risque de regression jamais teste jusque-la.

**Bonne nouvelle trouvee en explorant** : le mecanisme demande existe deja et fonctionne - `REPEATABLE_JOBS` (`vars.conf`, ajoute le 2026-09-14) efface deja automatiquement le marqueur `.ok` avant le controle "deja fait" de `bin/order.sh`. Le vrai ecart : seuls les 3 jobs `BEAC_00X` y figuraient, alors que les 6 jobs "playbook" du 2026-09-16/17 (explicitement decrits comme "un job RUN pour X" au moment de leur creation) exigeaient encore un `rm state/*.ok` manuel a chaque nouveau test.

**Corrige (`vars.conf`, `REPEATABLE_JOBS`)** : elargi a `WAZ_050_VIRUSTOTAL_INTEGRATION`, `WAZ_051_IOC_CDB_LIST`, `WAZ_052_SSHD_CONFIG_WHODATA`, `WAG_007_ROOTCHECK`, `WAG_008_AUDITD_INTEGRATION`, `WAG_009_VT_WATCH_DIR` - reclassification manuelle, job par job, jamais automatique. Explicitement PAS ajoutes : `INFRA_010_SCHEDULER_INSTALL`, `WAZ_053_VT_WATCH_GUARDIAN`, `WAG_010_VT_WATCH_GUARDIAN` (installations UNE FOIS d'un timer systemd - comportement `BUILD` correct malgre leur nom, les rejouer sans raison recreerait juste le meme timer).

**`orchestrator.sh` (boucle automatique) verifie et volontairement PAS modifie** : les 6 jobs ci-dessus ont tous un `IN_COND` de type `*_MANUAL_GATE` (`VIRUSTOTAL_MANUAL_GATE`, `WAZ_PURGE_MANUAL_GATE`) - confirme par grep sur les 296 lignes qu'AUCUN job ne produit cette condition, donc ces jobs ne sont JAMAIS eligibles via la resolution automatique de dependances, uniquement via `bin/order.sh` (Force-Start) - exactement le sens du tag `[MANUEL UNIQUEMENT]` deja present dans leur description. Le risque theorique "un job RUN qui se redeclenche indefiniment a chaque passage de l'orchestrateur" ne s'applique donc a aucun cas reel existant.

**Verifie** : `bash -n vars.conf` propre. Grep de controle avant modification : aucun des 6 `JOB_ID` n'etait deja present dans `REPEATABLE_JOBS` (pas de doublon introduit). Jamais teste en conditions reelles a l'instant de cette entree - a confirmer en rejouant `WAZ_050`/`WAG_009` via `bin/order.sh` SANS aucun `rm` manuel prealable.

## 2026-09-18 (suite) - Audit systematique complet des 296 jobs, pas seulement les 6 evidents

**Demande explicite, poussant a aller plus loin** : "je suis en train de vous dire que tout ce que j'avais fait a l'epoque n'est pas a l'image de Control-M et c'est ca qu'on veut corriger... vous direz non parce que vous avez deja croise BLD et RUN dans mes travaux ?" - juste apres le snapshot de securite (tag git + bundle local) demande explicitement en prevision d'un changement plus large. Reproche legitime : la premiere passe (6 jobs playbook) etait correcte mais **incomplete** - pas un vrai audit des 296 lignes.

**Audit systematique reel effectue** (script Python, jamais une relecture visuelle) : recherche de mots-cles de recurrence/bascule ("permanent", "continue", "hebdomadaire", "quotidien", "bascule", "trigger", "guardian", "sonde"...) dans la colonne DESC des 296 lignes, croisee avec `REPEATABLE_JOBS` - 36 candidats trouves, chacun verifie individuellement (jamais en bloc) en lisant le script reel :

- **Faux positifs confirmes** (le mot "permanent"/"continue" decrit l'EFFET durable d'une config UNIQUE, jamais une re-execution necessaire) : `ES_004/011/012/042/059/059_FINAL`, `KB_013`, `LS_036_FINAL`, `FB_022`, `MB_007/008/021`, `WAZ_005/006/006B`, `WAZ_017B/C`, `WAZ_035A_PAUSE_DEP_JOBS`, `WAZ_014D_ALERTS_RETENTION` (politique ISM native, s'applique seule pour toujours), `LS_030`/`FB_016` (crash-tests uniques de la chaine de construction). Aucun changement.
- **Jobs "guardian" corrects tels quels** (installent LEUR PROPRE timer systemd UNE FOIS - la recurrence vit dans le timer installe, jamais dans une re-execution du job WEF) : `INFRA_003/004/005`, `INFRA_010_SCHEDULER_INSTALL`, `WAZ_041_ALERT_CANARY`, `WAZ_053_VT_WATCH_GUARDIAN`, `WAG_010_VT_WATCH_GUARDIAN`, `PKI_012_SERVE_CA_HTTP`. Aucun changement.
- **5 vrais oublis trouves, corriges (`vars.conf`, `REPEATABLE_JOBS` etendu)** :
  - **`WAZ_035_KIBANA_TRIGGER` / `WAZ_039_WAZUH_TRIGGER`** : **bug latent reel confirme en lisant le script**, pas suppose. Leur propre description dit deja "rejouable a la demande via bin/order.sh" et leur script efface bien lui-meme TOUTE la cascade en aval (`WAZ_035_KIBANA_TRIGGER.sh:40-42` : `for cond in WAZ_DEP_JOBS_PAUSED WAZ_INDEXER_DATA_CUT ...; do rm -f ...; done`) - mais jamais SA PROPRE condition (`WAZ_KIBANA_TRIGGERED`/`WAZ_WAZUH_TRIGGERED`). Consequence reelle avant ce correctif : la bascule Kibana<->Wazuh, documentee comme "rejouable a la demande" depuis le 2026-09-03, ne l'etait en verite que la PREMIERE fois - `bin/order.sh` aurait refuse tout second declenchement sans un `rm state/WAZ_KIBANA_TRIGGERED.ok` manuel, exactement le defaut denonce.
  - **`WAZ_042_INDEXER_UNLOCK`, `WAZ_043_DASHBOARD_FIELDS_REFRESH`, `WAZ_044_VD_SAFE_RETRY`** : deja commentes "point #9 de la mission - jobs RUN rejouables" dans leur propre fichier depuis le 2026-08-31 - une decision deja prise a l'epoque, jamais reportee dans `REPEATABLE_JOBS` jusqu'a cet audit.
...
## 2026-09-22 (suite) - PB-009 revu : `log_format=command` abandonne, calendrier natif WEF a la place

**Constate en reel sur wef-elk-core**, apres reparation du fichier `local_rules.xml` (voir entree precedente) : la regle 100300 restait tout de meme sans aucune alerte, meme apres plusieurs cycles de scan confirmes reellement executes (`ps aux` montrant le process `nmap` actif). Diagnostic pousse (contenu distinctif du scan absent de `alerts.json` sous N'IMPORTE QUELLE regle, `archives.json` desactive donc invisible autrement) : le mecanisme `<localfile><log_format>command</log_format><frequency>` de Wazuh n'a jamais livre son contenu a `analysisd`, pour une cause non confirmee avec certitude dans cet environnement (comportement interne pas assez verifiable pour conclure sans deviner - honnete plutot que suppose corrige a l'aveugle).

**Decision** : abandon de `log_format=command`, remplace par un mecanisme deja PROUVE fiable ce soir - le calendrier natif WEF (`bin/scheduler.sh`/`schedules.csv`, teste fonctionnellement le 2026-09-18) qui execute un nouveau job (`WAZ_056_NMAP_SCAN_RUN.sh`) ecrivant dans `/var/log/wef-nmap-scan.log`, surveille par Wazuh en `<log_format>syslog</log_format>` - EXACT MEME mecanisme deja visible et fonctionnel pour `/var/ossec/logs/active-responses.log` depuis le debut de la soiree (confirme dans `ossec.log`).

**`WAZ_055_NMAP_SCAN_INTEGRATION.sh` reduit** a l'installation seule (localfile syslog + regle 100300, une fois). **Nouveau `WAZ_056_NMAP_SCAN_RUN.sh`** execute le scan, `OUT_COND=NONE` (motif deja etabli pour les jobs planifies), `IN_COND=NMAP_SCHEDULE_GATE` - **gate deliberement jamais produite par aucun job**, empeche `orchestrator.sh` de le declencher automatiquement a chaque passe (puisque `job_done("NONE")` est toujours faux, un `IN_COND` reellement satisfait l'aurait rendu eligible a CHAQUE passage de l'orchestrateur - confirme par lecture de `orchestrator.sh`/`lib/commun.sh`). `bin/scheduler.sh` ne verifie jamais `IN_COND` (confirme par lecture ligne a ligne) - seul lui declenche ce job, via `schedules.csv` (`0 * * * *`, toutes les heures).

**Bug trouve en verifiant** (jamais laisse passer) : la premiere version de la ligne `WAZ_056` dans `jobs_table.csv` contenait une virgule a l'interieur du champ `DESC` non entre guillemets - aurait casse les 5 parseurs `IFS=','` existants (meme classe de bug deja documentee le 2026-09-18 pour la conception de `schedules.csv`). Detecte par l'audit systematique deja en place (`awk -F',' 'NR>1{print NF}'`, attendu uniforme a 8) avant tout commit - corrige avant publication.
